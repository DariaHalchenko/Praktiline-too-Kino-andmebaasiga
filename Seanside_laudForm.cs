...
namespace Praktiline_too_Kino_andmebaasiga
{
    public partial class Seanside_laudForm : Form
    {
        ...
        public void NaitaSeansid()
        {
            ...
            cmd = new SqlCommand("SELECT Seansid.Id, Kinolaud.Filmi_nimetus, Saal.Saal_Id, Seansid.Start_time, Seansid.Lopp_time FROM Seansid INNER JOIN Kinolaud ON Seansid.Kinolaud_Id = Kinolaud.Id INNER JOIN Saal ON Seansid.Saal_Id = Saal.Id", conn);
        ...
        private void Lisa_btn_Click(object sender, EventArgs e)
        {
            if (kinolaud_cb.SelectedItem != null && saal_cb.SelectedItem != null)
            {
                ...
                {
                    conn.Open();
                    cmd = new SqlCommand("INSERT INTO Seansid (Kinolaud_Id, Saal_Id, Start_time, Lopp_time) VALUES (@kinolaud_id, @saal_id, @start_time, @lopp_time)", conn);
                    cmd.Parameters.AddWithValue("@kinolaud_id", kinolaud_cb.SelectedItem);
                    cmd.Parameters.AddWithValue("@saal_id", saal_cb.SelectedItem);
                    cmd.Parameters.AddWithValue("@start_time", start_time_dtp.Value);
                    cmd.Parameters.AddWithValue("@lopp_time", lopp_time_dtp.Value); // Changed the parameter here
                    cmd.ExecuteNonQuery();
                    conn.Close();
        ...
        private void Uuenda_btn_Click(object sender, EventArgs e)
        {
            ...
            {
                conn.Open();
                cmd = new SqlCommand("UPDATE Seansid SET Kinolaud_Id = @kinolaud_id, Saal_Id = @saal_id, Start_time = @start_time, Lopp_time = @lopp_time WHERE Id = @id", conn); // Changed field name to Lopp_time
                cmd.Parameters.AddWithValue("@id", ID);
                cmd.Parameters.AddWithValue("@kinolaud_id", kinolaud_cb.SelectedItem);
                cmd.Parameters.AddWithValue("@saal_id", saal_cb.SelectedItem);
                cmd.Parameters.AddWithValue("@start_time", start_time_dtp.Value);
                cmd.Parameters.AddWithValue("@lopp_time", lopp_time_dtp.Value); // Changed parameter name to lopp_time
                cmd.ExecuteNonQuery();
                conn.Close();
        ...
        private void DataGridView_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            kinolaud_cb.SelectedItem = dataGridView.Rows[e.RowIndex].Cells["Filmi_nimetus"].Value.ToString();
            saal_cb.SelectedItem = dataGridView.Rows[e.RowIndex].Cells["Saal_Id"].Value.ToString();
            start_time_dtp.Value = Convert.ToDateTime(dataGridView.Rows[e.RowIndex].Cells["Start_time"].Value);
            lopp_time_dtp.Value = Convert.ToDateTime(dataGridView.Rows[e.RowIndex].Cells["Lopp_time"].Value); // Changed here
        }
    }
}
