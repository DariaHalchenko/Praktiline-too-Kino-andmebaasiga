    ...
    public partial class KinolaudForm : Form
    {
        ...
        private void Uuenda_btn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                conn.Open();
                cmd = new SqlCommand("UPDATE Kinolaud SET Filmi_nimetus=@filmi_nimetus, Aasta=@aasta, Poster=@poster, Saal=@saal WHERE Id=@id", conn);
                cmd.Parameters.AddWithValue("@id", ID);
                ...
                cmd.Parameters.AddWithValue("@saal", saal_cb.Text); // Используем правильный параметр для зала
                cmd.ExecuteNonQuery();
        ...
        private void Lisa_btn_Click(object sender, EventArgs e)
        {
            if (filmi_nimetus_txt.Text.Trim() != string.Empty && aasta_txt.Text.Trim() != string.Empty)
            {
                try
                {
                    ...
                    cmd = new SqlCommand("INSERT INTO Kinolaud (Filmi_nimetus, Aasta, Poster, Saal) VALUES (@filmi_nimetus, @aasta, @poster, @saal)", conn);
                    cmd.Parameters.AddWithValue("@filmi_nimetus", filmi_nimetus_txt.Text);
                    cmd.Parameters.AddWithValue("@aasta", aasta_txt.Text);
                    cmd.Parameters.AddWithValue("@poster", filmi_nimetus_txt.Text + extension);

                    cmd.Parameters.AddWithValue("@saal", ID);
        ...
        private void DataGridView_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            {
                poster_pb.Image = Image.FromFile(Path.Combine(Path.GetFullPath(@"..\..\Poster"), "default.jpg"));
            }
        }
    }
}